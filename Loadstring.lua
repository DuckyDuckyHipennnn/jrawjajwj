...
local SaveManager = {} do
    ...
    function SaveManager:Save(name)
        local fullPath = 'zealon_friday_autoplayer/configs/' .. name .. '.json'
    ...
    function SaveManager.Refresh()
        local list = listfiles('zealon_friday_autopayer/configs')
...
Groups.Credits = Tabs.Miscellaneous:AddRightGroupbox('Credits')
    ...
    addRichText(Groups.Credits:AddLabel('<font color="#3020ff">zealon</font> - script fixer'))
...
if type(readfile) == 'function' and type(writefile) == 'function' and type(makefolder) == 'function' and type(isfolder) == 'function' then
    makefolder('zealon_friday_autopayer')
    makefolder('zealon_friday_autopayer\\configs')
...
do
    ...
    themeManager:SetFolder('zealon_friday_autoplayer')
